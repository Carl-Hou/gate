...
public class SetProperty extends AbstractFunction {
    ...
    static {
        ...
        desc.add("Value of property"); //$NON-NLS-1$
        desc.add("Return Original Value of property (default false) ?"); //$NON-NLS-1$
    }
    ...
    public String executeRecursion()
        ...
        if (returnValue) { // Only obtain and cast the return if needed
            return GateProps.setProperty(propertyName, propertyValue);
        } else {
            GateProps.setProperty(propertyName, propertyValue);
            return "";
        }
    }
