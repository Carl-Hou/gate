...
package org.gate.gui.sslmanager;
...
public class JsseSSLManager extends SSLManager {
    ...
    private static final String DEFAULT_SSL_PROTOCOL =
        GateProps.getProperty("https.default.protocol","TLS"); // $NON-NLS-1$ // $NON-NLS-2$

    // Allow reversion to original shared session context
    private static final boolean SHARED_SESSION_CONTEXT =
            GateProps.getProperty("https.sessioncontext.shared",false); // $NON-NLS-1$

    /**
     * Characters per second, used to slow down sockets
     */
    public static final int CPS = GateProps.getProperty("httpclient.socket.https.cps", 0); // $NON-NLS-1$
