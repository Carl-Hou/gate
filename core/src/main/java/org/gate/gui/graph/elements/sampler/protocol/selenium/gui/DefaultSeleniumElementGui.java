...
public class DefaultSeleniumElementGui extends JPanel implements GraphElementPropertiesGui, SeleniumConstantsInterface {
...
//    PropertiesTable returnValueVariablesNameTable = new PropertiesTable(new PropertiesTableModel());
    ...
    public DefaultSeleniumElementGui(){
        ...
        JPanel methodPanel = GuiUtils.getPanel("Method:", new VerticalLayout());
        JPanel methodSelectPanel = new JPanel(new GridLayout(1,2));
        methodSelectPanel.add(new JLabel("Name:"));
        ...
        JPanel methodArgumentsPanel = GuiUtils.getPanel("Arguments:", new GridLayout(1,1));
        methodArgumentsPanel.add(argumentPropertiesTable);
        methodPanel.add(methodArgumentsPanel);

        JPanel simplerArgumentPanel = GuiUtils.getPanel("Sampler Arguments:");
        simplerArgumentPanel.setLayout(new GridLayout(1,1));
        simplerArgumentPanel.add(defaultPropertiesTable);
        elementMethod.setRightComponent(methodPanel);
        elementMethod.setLeftComponent(simplerArgumentPanel);
        add(elementMethod);
    ...
    void updateTableEditors(){
        try {
//            defaultPropertiesTable.constraintNotEmpty(PN_DriverId);
            applyTableEditors();
        }catch (Throwable t){
    ...
    void applyTableEditors() {
        defaultPropertiesTable.setComboBox(ConditionChecker.PN_LocatorType, locatorTypeComboBox);
