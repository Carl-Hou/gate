...
public class JSONExtractorGui extends DefaultExtractorGui implements GraphElementPropertiesGui, ExtractorConstantsInterface {

    protected Logger log = LogManager.getLogger(this.getName());
    JComboBox jsonExtractorTypesComboBox = new JComboBox(JSON_ExtractorTypes);
    void onInputSourceSelected(AbstractExtractor extractor){
        extractor.putProp(TestElement.NS_DEFAULT, Extractor_Type, JSON_ExtractorType_JSONPath);
    }

    void updateTableEditors(){
        defaultPropertiesTable.setComboBox(Extractor_Type, jsonExtractorTypesComboBox);
    }
